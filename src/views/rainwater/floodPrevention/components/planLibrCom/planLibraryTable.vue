    ...
    <div class="app-container">
            ...
            <div class="titleBottom">
                <pro-crud ref="crudRef" :index="index" v-model="form" v-model:search="serachForm" :columns="columns"
                    ...
                    :page-sizes="[10, 20, 30, 40, 50]" label-width="100px" direction="vertical" @search="searchSubmit"
                    ...
                    <template #form-startEndTime>
                        <PlanTime :disabled="false"/>
                    </template>
                    <template #form-personGuaranteeDTOS>
                        <PersoncomTable :tableData="perTableData" :disabled="false"/>
                    </template>
                    <template #form-emergencyMaterialsDTOS>
                        <MaterialComTable :tableData="matTableData" :disabled="false"/>
                    </template>
                    <template #crud-detail>
                        ...
                        <template #form-personGuaranteeDTOS>
                            <PersoncomTable :tableData="perTableData" :disabled="true"/>
                        </template>
                        <template #form-emergencyMaterialsDTOS>
                        <MaterialComTable :tableData="matTableData" :disabled="true"/>
                    </template>
                        </pro-form>
                    </template>
                </pro-crud>
...
<script setup name="planLibraryTable">
...
import PersoncomTable from './personcomTable'
import MaterialComTable from './materialComTable'
...
let tableData = ref([]);
let perTableData = ref([]);
let matTableData = ref([]);
let evacuateLineTable = ref([]);
let personGetListUrl = '/floodhead/queryPersonGuarantee';
let materialGetListUrl = '/material/queryEmergencyMaterials';
...
const beforeOpen = defineCrudBeforeOpen((done, type, row) => {
    if (type === 'edit' || type === 'detail' ) {
        getRequest(props.detailUrl, { [props.code]: row[props.code] }).then(res => {
            // debugger
            form.value = res.data;
            perTableData.value = res.data.personGuaranteeVOS
            matTableData.value = res.data.emergencyMaterialsVOS
        })
    } else {
        form.value = {};
        perTableData.value = [];
...
const submit = defineCrudSubmit((close, done, type, isValid, invalidFields) => {
    console.log(matTableData.value, 'matTableData')
    console.log(perTableData.value, 'perTableData')
    if (isValid) {
        let obj = {
            ...form.value,
            personGuaranteeDTOS: perTableData.value,
            emergencyMaterialsDTOS: matTableData.value,
            evacuateLineDTOS: evacuateLineTable.value,
        }
        if (type == 'add') {
