    ...
    <div class="app-container">
            ...
            <div class="titleBottom">
                <pro-crud ref="crudRef" :index="index" v-model="form" v-model:search="serachForm" :columns="columns"
                    ...
                    :before-close="beforeClose" height="550" width="70%" @search-reset="searchReset" @submit="submit"
                    :gutter="20" empty-text="暂无数据"
                    @delete="deleteRow">
                    <template #search-menu-right="{ size }" v-if="isAdd">
...
<script setup name="floodCommonTable">
...
const props = defineProps(['title', 'getListUrl', 'addUrl', 'editUrl', 'deleteUrl', 'detailUrl', 'code', 'formColumns', 'columns', 'menu', 'isAdd', 'stType']);
...
// 详情表单
let detailColumns = defineFormColumns(computed(() => [...props.formColumns].map(r=>({...r,props:{...r.props,disabled:true},rules:[{ required: false}]}))))
...
// 获取表格数据
let getTableData = () => {
    postRequest(props.getListUrl, {...serachForm.value, sttp: props.stType}).then(res => {
...
const submit = defineCrudSubmit((close, done, type, isValid, invalidFields) => {
    if (isValid) {
        if (type == 'add') {
            postRequest(props.addUrl, form.value).then(res => {
                ElMessage({
                    message: '添加成功',
                    type: 'success'
                });
            ...
            putRequest(props.editUrl, form.value).then(res => {
                ElMessage({
                    message: '修改成功',
                    type: 'success'
                });
